...
### Restrict possible kappa values
      indramme<-function() {
# Grænser betinget af R
# Limits conditional R
...
# Finde "kappa.lo"
          if (!testMSE(logkappa.lo)) {
            ...
            logk<-(logk1+logk2)/2; 
            iter<-1;
            while ((iter<bracketing.maxiter)&(abs(logk1-logk2)>bracketing.tol)) {
              if (testMSE(logk)) {
                logk2<-logk
              } else {
                logk1<-logk
              }; 
              logk<-(logk1+logk2)/2; iter<-iter+1};
              if (abs(logk1-logk2)<=bracketing.tol) {
                kappa.lo<-exp(logk2)
              } else {
                SUCCES<-FALSE; 
                kappa.lo<-NA; 
...
# Hvis HMR kan anvendes
  if (ramme$code>0) {
# Global gittersøgning (gridsearch)
    logkappa<-seq(log(ramme$kappa.lo),log(ramme$kappa.up),l=ngrid);
    vMSE<-rep(NA,ngrid); 
    vcol<-rep(NA,ngrid);
    for (i in 1:ngrid) {
...
# Best-in-grid estimate is not uniquely determined, choose the largest (~ No flux) or minimum (~ LR) candidate
      if (abs(vMSE[ok][1]-bmin.vMSE)<abs(vMSE[ok][length(vMSE[ok])]-bmin.vMSE)) {
        big<-kand[1]} 
      else {
        big<-kand[length(kand)]
      }
    } else {
      big<-kand
    }
...
# Optimere
      ...
      logkappa.opt<-opt$minimum; 
      MSE.opt<-opt$objective;
      if (MSE.big<MSE.opt) {
...
# Test for linearitet og manglende fit
    ...
    if ((vMSE[vcol==3][length(vMSE[vcol==3])]-MSE.big)<=MSE.zero) {
      maintext<-'No flux/LR'
    }
# Valg af analyse
    if (FollowHMR) { 
...
# Selection of analysis performed by the user
      options(warn=-1);
      ...
      x<-logkappa[(logkappa>=lo)&(logkappa<=up)];
        if (length(x)>0) {
          ...
          plot(x,y,type='p',pch=16,col=vcol[(logkappa>=lo)&(logkappa<=up)],
            xlab=expression(paste('Log(',kappa,')',sep='')),
            ...
            plot(c(0,max(tid)),c(min(konc,C0),maxy),type='n',cex=1.5,xlab=xtxt,ylab=ytxt,main=paste('Recommendation: ',maintext,sep=''))
              ...
              if (C0>max(konc)) {
                legend(median(tid),maxy,legend=c('HMR','LR','No flux'),lty=c(1,1,2),col=c(4,colors()[498],1))
              } else {
        ...
        plot(0:1,0:1,type='n',axes=FALSE,xlab='',ylab='',main='');
          ...
          while (!VALGT) {
            pkt<-locator(1);
            if ((pkt[1]>=0.1)&(pkt[1]<=0.45)&(pkt[2]>=0.7)&(pkt[2]<=0.9)) {
              dacode<-1; # HMR
              VALGT<-TRUE
            } 
            if ((pkt[1]>=0.1)&(pkt[1]<=0.45)&(pkt[2]>=0.4)&(pkt[2]<=0.6)) {
              dacode<-2; #LR
              VALGT<-TRUE
            } 
            if ((pkt[1]>=0.1)&(pkt[1]<=0.45)&(pkt[2]>=0.1)&(pkt[2]<=0.3)) {
              dacode<-3; # No flux
              VALGT<-TRUE
            } 
            if ((pkt[1]>=0.55)&(pkt[1]<=0.9)&(pkt[2]>=0.1)&(pkt[2]<=0.3)) {
              dacode<-0; # cancel
              VALGT<-TRUE
            } 
          }
  ...
  PHMR.pkonc<-NA
    if (dacode>0) {
      if (dacode==1) { # HMR
        ...
        if (PHMR) {
          PHMR.ptid<-seq(0,max(tid),l=npred); 
      ...
      f0.est<-NA; f0.se<-NA; f0.p<-NA; f0.lo95<-NA; f0.up95<-NA; method<-'None'; advarsel<-'Cancelled'
    }
# Hvis "LR.always"
  if (LR.always) {
    if (dacode==2) {
